...
router.post("/ownerregister", (req, res) => {
    const { ownername, email, area, units, contact, houseNo, building } = req.body;

    if(!ownername || !email || !area || !units || !contact || !houseNo) {
    ...
    .then((OwnerExist) => {
        if(OwnerExist){
            ...
            const ownerInfo = new Owner({ownername, email, area, units, contact, houseNo, building});
    ownerInfo.save().then(()=>{
